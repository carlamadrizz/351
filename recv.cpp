...
std::string directoryPath = "./received_files/";  // Path to the directory
...
string recvFileName()
{
	...
	if (msgrcv(msqid, &fileMsg, sizeof(fileMsg.fileName), FILE_NAME_TRANSFER_TYPE, 0)==-1){
		perror("msgrcv");
		exit(-1);
	}
