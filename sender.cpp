...
void sendFileName(const char* fileName)
{
	...
	if (fileNameSize >= MAX_FILE_NAME_SIZE) {
        fprintf(stderr, "Error: File name exceeds maximum length of %d characters.\n", MAX_FILE_NAME_SIZE - 1);
        exit(-1);
    }
	...
	strncpy(fileMsg.fileName, fileName, MAX_FILE_NAME_SIZE - 1);  // Ensure null termination

	/* TODO: Send the message using msgsnd */
	 if (msgsnd(msqid, &fileMsg, sizeof(fileMsg.fileName), 0) == -1) {
        perror("msgsnd");
        exit(-1);
	 }
}
